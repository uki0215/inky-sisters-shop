...
async function seedDatabase() {
  ...
  if (catCount > 0) {
    return { success: true, message: 'Өгөгдөл аль хэдийн байна.' };
  }

  // Create Categories
  const catPens = await db.category.create({
    data: { name: 'Үзэг & Балушка', slug: 'pens-markers', icon: 'PenTool' }
  });
  const catNotebooks = await db.category.create({
    data: { name: 'Дэвтэр & Төлөвлөгч', slug: 'notebooks-planners', icon: 'BookOpen' }
  });
  const catArt = await db.category.create({
    data: { name: 'Будаг & Зургийн хэрэгсэл', slug: 'art-drawing', icon: 'Palette' }
  });
  const catOffice = await db.category.create({
    data: { name: 'Албан тасалгааны хэрэгсэл', slug: 'office-supplies', icon: 'Briefcase' }
  });
  const catSchool = await db.category.create({
  ...
  for (const p of products) {
    const createdProd = await db.product.create({ data: p });
    await db.productHistory.create({
      data: {
        productId: createdProd.id,
        changeType: 'INITIAL',
        description: 'Анхны бараа бүртгэл',
        newPriceMnt: createdProd.priceMnt,
        newCostMnt: createdProd.costMnt,
        newCostYuan: createdProd.costYuan,
        newYuanRate: createdProd.yuanRate,
        addedStock: createdProd.stock,
        newStock: createdProd.stock,
        note: 'Системийн анхны өгөгдөл',
      },
    });
  }

  // Create Bank QR codes
  ...
  if (promoCount === 0) {
    await db.promotionBanner.create({
      data: {
        ...
        discountCode: 'INKY2026',
        active: true,
      }
    });
  }

  return { success: true, message: 'Анхны ангилал ба барааны өгөгдөл амжилттай үүсгэгдлээ.' };
}
